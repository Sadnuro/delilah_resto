...
router.post('/user', auth.validateFormat, auth.validateUser, async (req, res) => {   // 
  ...
  try {
    ...
    if (result!=undefined) {
      ...
      es.status(500).json({success: false, msg: error.message});
    }
  }catch(error){
...
async function deleteOrdersOfUser (IdUser){
  ...
  data = {IdUser: IdUser};
  const toDelete = await actions.Select("SELECT id as idOrden FROM ordenes WHERE IdUser=:IdUser", data);
  ...
  if (toDelete.length>0){
      ...
      for (let index = 0; index < toDelete.length; index++) {
        const res_DO = await actions.Delete(`DELETE FROM detallesordenes WHERE idOrden=:idOrden`, {idOrden: toDelete[index].idOrden});
      }
      const res_O = await actions.Delete(`DELETE FROM ordenes WHERE IdUser=:IdUser`, data );
      const SET_FK = await actions.query("SET FOREIGN_KEY_CHECKS = 1");

      return {success: true, msg: 'ORDERS_AND_DETAILSORDERS_DELETED', ordersDeleted: toDelete.length, details: toDelete};
  } else {
...
router.delete("/user/:id", auth.authAdmin, async (req, res) => {
  ...
  if (exists.length === 0) {
    ...
    try {
      ...
      console.log("RESULT OF DELETE ORDERS AND DETAILSORDERS:", resultDeleteOrders);
